define(['knockout','ojs/ojcore', 'jquery', 
 ...
 function tableViewModel(context){
     ...
     "]"; 
            ...
            $.getJSON("http://adc6141041.us.oracle.com:7101/JetCCA-JetComposite-context-root/jet-composites/object-jet/dummydata.json", {PARAM_ID : objectId}, function(data){
            ...
            $.getJSON("http://adc6141041.us.oracle.com:7101/JetCCA-JetComposite-context-root/jet-composites/object-jet/dummydataLoc.json", {PARAM_ID : objectId}, function(data){
                 $.map(data.items, function(val, i){
                // console.log("Hello: "+val.headerValues[0].column+"-"+val.headerValues[1].name+"-"+val.headerValues[2].name+"-"+val.headerValues[3].name+"-"+val.headerValues[4].name+"-"+val.headerValues[5].name);      
        ...
        self.reloadData = function (selectedVal){        
                self.tabledata.removeAll();
                $.getJSON("http://adc6141041.us.oracle.com:7101/JetCCA-JetComposite-context-root/jet-composites/object-jet/dummydata.json", {PARAM_ID : selectedVal}, function(data){
                    
                 $.map(data.items, function(val, i){
                  if(val.headerValues[10].column==selectedVal){
                        self.tabledata.push(val);
                    }
                 });
                 self.headerTemplate = 'th_template_dept';
